...
spec=importlib.util.spec_from_file_location('engine',ROOT/'server/engine.py');E=importlib.util.module_from_spec(spec);spec.loader.exec_module(E)
class EngineTests(unittest.TestCase):
 ...
 def test_integration_two_snapshots_and_dedup(self):
  with tempfile.TemporaryDirectory() as d:
   db=str(pathlib.Path(d)/'test.sqlite');c=sqlite3.connect(db)
   src=(ROOT.parents[1]/'public_html/api/live90/core.php').read_text();sql=src.split('$db->exec("')[1].split('");')[0];c.executescript(sql)
   c.execute('INSERT INTO prematch(match_id,recorded_at,kickoff,data) VALUES(?,?,?,?)',('123',self.b['recorded_at'],self.ko,json.dumps(self.b)))
   old=(self.now-dt.timedelta(seconds=45)).isoformat()
   c.execute('INSERT INTO samples(cycle_id,match_id,received_at,data) VALUES(?,?,?,?)',('a','123',old,json.dumps(dict(self.r,minute=59))));c.commit();E.run(db)
   self.assertEqual(c.execute('SELECT COUNT(*) FROM signals').fetchone()[0],0)
   c.execute('INSERT INTO samples(cycle_id,match_id,received_at,data) VALUES(?,?,?,?)',('b','123',self.now.isoformat(),json.dumps(self.r)));c.commit();E.run(db);E.run(db)
   self.assertEqual(c.execute('SELECT COUNT(*) FROM signals').fetchone()[0],1)
   self.assertEqual(c.execute('SELECT delivery FROM signals').fetchone()[0],'disabled');c.close()
 def test_changed_score_needs_confirmation(self):
  with tempfile.TemporaryDirectory() as d:
   db=str(pathlib.Path(d)/'test.sqlite');c=sqlite3.connect(db);src=(ROOT.parents[1]/'public_html/api/live90/core.php').read_text();c.executescript(src.split('$db->exec("')[1].split('");')[0])
   c.execute('INSERT INTO prematch(match_id,recorded_at,kickoff,data) VALUES(?,?,?,?)',('123',self.b['recorded_at'],self.ko,json.dumps(self.b)))
   c.execute('INSERT INTO samples(cycle_id,match_id,received_at,data) VALUES(?,?,?,?)',('a','123',(self.now-dt.timedelta(seconds=45)).isoformat(),json.dumps(self.r)))
   self.r['score']['h']=1;self.r['quotes'][0]['line']=1.5
   c.execute('INSERT INTO samples(cycle_id,match_id,received_at,data) VALUES(?,?,?,?)',('b','123',self.now.isoformat(),json.dumps(self.r)));c.commit();E.run(db)
   self.assertEqual(c.execute('SELECT COUNT(*) FROM signals').fetchone()[0],0);c.close()
