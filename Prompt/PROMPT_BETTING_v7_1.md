# PROMPT BETTING v7.1 — StratEdge Pronos (COMPRESSÉ)
## 22/03/2026 — 49 règles, 11 échecs documentés | Post-mortem Santos/Cruzeiro intégré
...
Scanner Winamax → Betclic → Oddschecker → **Stake.bet (cote réelle prioritaire)**. Recalculer EV avec cote réelle. EV réel < +3% → SKIP. **R43 : JAMAIS de cote estimée en verdict final.** Si cote réelle indisponible → mentionner "cote non vérifiée" + cap ⭐⭐⭐ max. Les estimations de l'analyste sont systématiquement 20-30% au-dessus du marché. **Dropping odds (R39) :** si cote a baissé >10% en 24h → sharp money passé, value disparue. Si cote monte >10% → investiguer (blessure ? rotation ?). **Outil recommandé : The Odds API (the-odds-api.com) — endpoint `player_shots_on_target` dispo pour Big 5.** *Né de l'erreur Dortmund-Bayern (28/02) + erreur Santos (22/03).*
...
### Outils de vérification cotes (NOUVEAU v7.1)
**Priorité 1 (cotes réelles) :** Stake.bet (bookmaker principal), bet365, Unibet
**Priorité 2 (comparaison) :** oddschecker.com (compare 10+ books), sportsgambler.com (props joueur SOT avec cotes)
**Priorité 3 (API automatisée) :** The Odds API (the-odds-api.com) — plan gratuit 500 req/mois. Endpoint `player_shots_on_target` dispo pour Big 5 (EPL, Liga, Serie A, Bundesliga, Ligue 1, MLS). **Clé API = donnée SAFE** (lecture seule, pas d'accès compte). ≠ Token Stake (accès complet compte = JAMAIS partager).
