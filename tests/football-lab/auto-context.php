...
check(\StratEdgeLab\Context::parse($verdictResponse($verdict),'fixture')['decision'] === 'retained', 'Structured GPT verdict accepted with confirmed lineups');
$verdict['official_lineups'] = false;
check(\StratEdgeLab\Context::parse($verdictResponse($verdict),'fixture')['decision'] === 'pending', 'Unconfirmed lineups cannot produce retained verdict');
$verdict['official_lineups'] = true; $verdict['critical_unknowns'] = ['Gardien incertain'];
check(\StratEdgeLab\Context::parse($verdictResponse($verdict),'fixture')['decision'] === 'pending', 'Critical unknown prevents retained verdict');
$verdict['decision'] = 'excluded';
check(\StratEdgeLab\Context::parse($verdictResponse($verdict),'fixture')['decision'] === 'excluded', 'Sourced exclusion preserved');
