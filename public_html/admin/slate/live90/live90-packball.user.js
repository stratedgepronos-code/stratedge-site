...
function collect(){
 ...
 const side=cls==='inp-7'?'over':cls==='inp-8'?'under':null;
 const valid=side!==null&&/buts/.test(t)&&/premiere ligne/.test(t)&&!/(mi-temps|domicile|visiteur|equipe|corner)/.test(t);
 r.quotes.push({market:valid?'total_goals':'unmapped',side,period:valid?'FT':null,verified:valid,line:number(el.querySelector('.label')?.textContent),odds:number(el.querySelector('.odds-inplay')?.textContent),title,column:cls,bookmaker:'bet365',observed_at:new Date().toISOString()});
 });rows.push(r);
 });
 return {schema:2,collector_version:'1.0.0',cycle_id:crypto.randomUUID(),collected_at:new Date().toISOString(),source:'packball',page_rows:rows.length,odds_meta:meta,stat_headers:heads,rows};
}
