...
try {
    $db->query("SELECT pronostic FROM montante_foot_steps LIMIT 1");
} catch (Throwable $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if (!verifyCsrf($_POST['csrf_token'] ?? '')) {
        ...
        $nom = trim($_POST['nom'] ?? 'Montante Foot');
        ...
        $db->exec("UPDATE montante_foot_config SET statut = 'terminee' WHERE statut IN ('active','pause')");
        $stmt = $db->prepare("INSERT INTO montante_foot_config (nom, bankroll_initial, mise_depart, statut, date_debut) VALUES (?, ?, ?, 'active', ?)");
        $stmt->execute([$nom, $bankroll, $miseDepart, $dateDebut]);
        $newId = (int)$db->lastInsertId();
        $success = 'Nouvelle montante créée : ' . clean($nom);
        if ($newId) {
            $newConfig = $db->prepare("SELECT * FROM montante_foot_config WHERE id = ?");
        ...
        if ($montanteId && in_array($newStatut, ['active','pause','terminee'])) {
            $db->prepare("UPDATE montante_foot_config SET statut = ? WHERE id = ?")->execute([$newStatut, $montanteId]);
        ...
        if ($montanteId && $matchDesc !== '') {
            $maxStep = (int)$db->prepare("SELECT COALESCE(MAX(step_number),0) FROM montante_foot_steps WHERE montante_id = ?");
            $stmtMax = $db->prepare("SELECT COALESCE(MAX(step_number),0) FROM montante_foot_steps WHERE montante_id = ?");
            $stmtMax->execute([$montanteId]);
            $nextStep = (int)$stmtMax->fetchColumn() + 1;

            $stmt = $db->prepare("INSERT INTO montante_foot_steps (montante_id, step_number, match_desc, competition, pronostic, cote, mise, date_match, heure, analyse) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            if ($stepId && $config = $db->query("SELECT * FROM montante_foot_config WHERE id = " . (int)$montanteId)->fetch(PDO::FETCH_ASSOC)) {
                $newStep = $db->prepare("SELECT * FROM montante_foot_steps WHERE id = ?");
        ...
        if ($stepId && in_array($resultat, ['gagne','perdu','annule'])) {
            $step = $db->prepare("SELECT * FROM montante_foot_steps WHERE id = ?");
            $step->execute([$stepId]);
            $s = $step->fetch();

            if ($s) {
                $config = $db->prepare("SELECT * FROM montante_foot_config WHERE id = ?");
                ...
                $db->prepare("UPDATE montante_foot_steps SET resultat = ?, gain_perte = ?, bankroll_apres = ? WHERE id = ?")
        ...
        $montanteId = (int)($_POST['montante_id'] ?? 0);
        if ($montanteId) {
            $db->prepare("DELETE FROM montante_foot_steps WHERE montante_id = ?")->execute([$montanteId]);
            $db->prepare("DELETE FROM montante_foot_config WHERE id = ?")->execute([$montanteId]);
        ...
        if ($stepId) {
            ...
            if ($matchDesc !== '') {
                $db->prepare("UPDATE montante_foot_steps SET match_desc = ?, competition = ?, pronostic = ?, cote = ?, mise = ?, date_match = ?, heure = ?, analyse = ? WHERE id = ?")
        ...
        if ($stepId) {
            ...
            $stmt = $db->prepare("SELECT montante_id, step_number FROM montante_foot_steps WHERE id = ?");
            $stmt->execute([$stepId]);
            ...
            if ($row) {
                ...
                // Supprimer l'étape
                $db->prepare("DELETE FROM montante_foot_steps WHERE id = ?")->execute([$stepId]);

                // Décrémenter les step_number des étapes qui étaient après
                $db->prepare("UPDATE montante_foot_steps SET step_number = step_number - 1 WHERE montante_id = ? AND step_number > ?")
                   ->execute([$mid, $deletedNum]);

                // Recalculer les bankroll_apres en cascade
                $cfg = $db->prepare("SELECT * FROM montante_foot_config WHERE id = ?");
                $cfg->execute([$mid]);
...
$config = $db->query("SELECT * FROM montante_foot_config ORDER BY id DESC LIMIT 1")->fetch();
$steps = [];
if ($config) {
    $stmtSteps = $db->prepare("SELECT * FROM montante_foot_steps WHERE montante_id = ? ORDER BY step_number DESC");
    $stmtSteps->execute([$config['id']]);
    $steps = $stmtSteps->fetchAll();
}
$toutesMontantes = $db->query("SELECT * FROM montante_foot_config ORDER BY id DESC")->fetchAll();
?>
<!DOCTYPE html>
<html lang="fr">
<head>
...
<div class="main">
  ...
  <div class="card">
    ...
    <form method="post">
      ...
      <input type="hidden" name="montante_id" value="<?= (int)$config['id'] ?>">
      <div class="form-grid">
        <div class="form-group"><label>Match</label><input type="text" name="match_desc" placeholder="Ex: PSG vs Marseille" required></div>
        <div class="form-group"><label>Compétition</label><input type="text" name="competition" placeholder="Ex: Ligue 1"></div>
        <div class="form-group" style="grid-column:1/-1;"><label>🎯 Pronostic</label><input type="text" name="pronostic" placeholder="Ex: Victoire PSG · Plus 2,5 buts · BTTS oui · ..."></div>
  ...
  $editStepRow = null;
  if ($editStepId) {
      $stEdit = $db->prepare("SELECT * FROM montante_foot_steps WHERE id = ?");
    ...
    <div style="background:rgba(0,212,106,0.06);border:1px solid rgba(0,212,106,0.25);border-radius:10px;padding:1rem;margin-bottom:1rem;">
      ...
      <form method="post" style="margin-top:1rem;">
        ...
        <div class="form-grid">
          ...
          <div class="form-group" style="grid-column:1/-1;"><label>🎯 Pronostic</label><input type="text" name="pronostic" value="<?= htmlspecialchars($editStepRow['pronostic'] ?? '') ?>" placeholder="Ex: Victoire PSG · Plus 2,5 buts · BTTS oui"></div>
