      ...
      <div class="promo-wrap" style="width:100%;margin-bottom:1rem;">
        <div style="display:flex;gap:.5rem;">
          <input type="text" id="promoCode" placeholder="Code promo" maxlength="30"
            style="flex:1;background:#0a0e17;border:1px solid rgba(57,255,20,0.2);border-radius:8px;padding:.6rem .8rem;color:#fff;font-family:'Share Tech Mono',monospace;font-size:.85rem;text-transform:uppercase;letter-spacing:1px;">
          <button type="button" onclick="checkPromo()" id="promoBtn"
            style="background:rgba(57,255,20,0.15);border:1px solid rgba(57,255,20,0.3);border-radius:8px;padding:.6rem 1rem;color:#39ff14;font-family:'Orbitron',sans-serif;font-size:.7rem;font-weight:700;cursor:pointer;letter-spacing:1px;">OK</button>
        </div>
        <div id="promoResult" style="margin-top:.4rem;font-size:.82rem;min-height:1.2rem;"></div>
      </div>
      <a href="/stripe-create-tennis.php" id="btnCB" class="o-btn o-btn-cb">💳 Carte bancaire — 15€</a>
      <a href="/nowpayments-create-abo.php?type=tennis" class="o-btn o-btn-cr">₿ Crypto (BTC, ETH, USDT...)</a>
      <button type="button" onclick="payPaysafe()" class="o-btn" style="background:linear-gradient(135deg,#0074d9,#00a8e8);color:#fff;border:none;width:100%;">🔒 Paysafecard — 15€</button>
    </div>
<script>
async function payPaysafe(){
  const btn=event.target;btn.disabled=true;btn.textContent='⏳ Redirection...';
  try{
    const fd=new FormData();fd.append('offre','tennis');
    const r=await fetch('/paysafe-create.php',{method:'POST',body:fd,credentials:'same-origin'});
    const d=await r.json();
    if(d.url){window.location.href=d.url;}
    else{alert('Erreur Paysafecard : '+(d.error||'inconnue'));btn.disabled=false;btn.textContent='🔒 Paysafecard — 15€';}
  }catch(e){alert('Erreur réseau');btn.disabled=false;btn.textContent='🔒 Paysafecard — 15€';}
}
async function checkPromo(){
  ...
  try{
    const fd=new FormData();fd.append('code',code);fd.append('offre','tennis');
    const r=await fetch('/api-check-promo.php',{method:'POST',body:fd,credentials:'same-origin'});
    const d=await r.json();
    if(d.ok){
      res.innerHTML='<span style="color:#39ff14">✅ '+d.label+' → <b>'+d.prix_final.toFixed(2)+'€</b></span>';
      btn.href='/stripe-create-tennis.php?promo='+encodeURIComponent(code);
      btn.textContent='💳 Carte bancaire — '+d.prix_final.toFixed(2)+'€';
    }else{
      res.innerHTML='<span style="color:#ff6b6b">❌ '+d.err+'</span>';
      btn.href='/stripe-create-tennis.php';btn.textContent='💳 Carte bancaire — 15€';
    }
